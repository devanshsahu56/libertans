...
const text2 = document.querySelector('.circless p');
text2.innerHTML = text2.innerText.split("").map(
  (char , i )=>`<span style="transform:rotate(${i*9.99}deg)">${char}</span> `
).join("");

const text3 = document.querySelector('#two p');
text3.innerHTML = text3.innerText.split("").map(
  (char , i )=>`<span style="transform:rotate(${i*9.99}deg)">${char}</span> `
).join("");


const text4 = document.querySelector('#three p');
text4.innerHTML = text4.innerText.split("").map(
  (char , i )=>`<span style="transform:rotate(${i*9.99}deg)">${char}</span> `
).join("");

const text5 = document.querySelector('#four p');
text5.innerHTML = text5.innerText.split("").map(
  (char , i )=>`<span style="transform:rotate(${i*9.99}deg)">${char}</span> `
).join("");


const text6 = document.querySelector('.circl p');
